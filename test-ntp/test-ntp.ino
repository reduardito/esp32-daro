...
void setup()
{
  ...
  dht.begin();
  if (!SD.begin(CD_PIN)){
    Serial.println("Error, no puede iniciarse la SD");
    return;
  }
}
...
//    u8g2.setFont(u8g2_font_ncenB08_tr);   
    ...
    if (record){
      //record.seek(EOF);
      record.println("Temp: " + temp + " Hum: " + hum + " " + date);
      record.close();
      Serial.print("Se escribio en el archivo");
    }else{
      Serial.println("error no se puede abrir el archivo");
    }
    temp = "";
    hum = "";    
  }
